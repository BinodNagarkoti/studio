...
const GetAIWebSearchStockReportOutputSchema = z.object({
  report: z.string().describe('The AI-driven stock weather report based on simulated web search findings.'),
...
1.  A comprehensive stock report: Detail your findings and provide an overall outlook for the stock. Consider its sector, recent performance trends (if general knowledge allows), any major known news or events, and general market perception.
...
3.  A confidence level: Provide a numerical value between 0.0 (no confidence) and 1.0 (complete confidence) in your assessment. Justify this confidence level briefly, considering the typical availability and reliability of information for such stocks.
